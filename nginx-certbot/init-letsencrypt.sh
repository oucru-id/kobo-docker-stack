...
DOMAINS=(kobo.lombokbarat.oucru-id.org kobocat.lombokbarat.oucru-id.org enketo.lombokbarat.oucru-id.org)
...
DOCKER_COMPOSE_CMD="$(which docker)"
CURL_CMD=$(which curl)
...
$DOCKER_COMPOSE_CMD  compose run --rm --entrypoint "\
...
echo "### Starting nginx ..."
$DOCKER_COMPOSE_CMD  compose up --force-recreate -d nginx_ssl_proxy
echo
...
$DOCKER_COMPOSE_CMD  compose exec nginx_ssl_proxy nginx -s reload
